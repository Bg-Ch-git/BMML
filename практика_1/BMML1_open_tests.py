...
import numpy as np
import time
import bogdan_chuzhinov_v2 as code
var_number = 2


# functions to test
ff = list()
ff.append(['pa', 'pb', 'pc', 'pd', 'pc_a', 'pc_b',
          'pc_d', 'pc_ab', 'pc_abd'])  # variant 1
ff.append(['pa', 'pb', 'pc', 'pd', 'pc_a', 'pc_b',
           'pb_a', 'pb_d', 'pb_ad'])   # variant 2
# ff.append(['pa', 'pb', 'pc', 'pd', 'pb_a', 'pc_a', 'pc_b', 'pb_d'])
ff.append(['pa', 'pb', 'pc', 'pd', 'pb_d', 'pb_ad']
          )                        # variant 3
# variant 3
ff.append(['generate'])
...
if var_number == 3:
    ...
    args_short.update({'pb_d': [d[:1, :]], 'pb_ad': [
                      a[:1], d[:1, :]], 'generate': [N, a[:1], b[:1]]})
    size_long.update({'pb_d': (blen, d.shape[0]), 'pb_ad': (
        blen, a.size, d.shape[0]), 'generate': (N, a.size, b.size)})
...
def test_distribution(f, model, arg_long, arg_short, size_long, size_short):
    ...
    if prob.shape != size_long:
        raise Exception('Function %s for model %d returns distribution probabilities of the wrong size: %s instead of %s.' % (
    ...
    if t > max_time:
        print('Warning: function %s for model %d is too slow: %.4f seconds.' %
              (f, model, t))
...
for f in ff[var_number-1]:
    test_distribution(f, models[0], args_long[f],
                      args_short[f], size_long[f], size_short[f])
    test_distribution(f, models[1], args_long[f],
                      args_short[f], size_long[f], size_short[f])

if var_number == 3:
    for f in ff[-1]:
        test_generate(f, models[0], args_long[f],
                      args_short[f], size_long[f], size_short[f])
        test_generate(f, models[1], args_long[f],
                      args_short[f], size_long[f], size_short[f])
