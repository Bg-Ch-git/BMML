...
def pc(params, model):
    ...
    if model == 2:
        ...
        a = np.arange(params['amin'], params['amax'] + 1)
        b = np.arange(params['bmin'], params['bmax'] + 1)
        A = binom.pmf(res_val, np.expand_dims(a, axis=0).T, params['p1'])
        B = binom.pmf(np.arange(clen - 1, -1, -1), np.expand_dims(b, axis=0).T, params['p2'])
        for i in range(clen):
            res_prob[i] = A[:,:i+1].dot(B[:,clen-1-i:].T).sum()
...
def pd(params, model):
    ...
    for c in val_c:
        res_prob[c:] += prob_c[c] * binom.pmf(res_val[:res_val.shape[0] - c], c, params['p3'])
...
def pc_b(b, params, model):
    ...
    if model == 2:
        ...
        B = binom.pmf(np.arange(clen - 1, -1, -1), np.expand_dims(b, axis=0).T, params['p2'])
        for i in range(clen):
            res_prob[i,:] = A[:,:i+1].dot(B[:,clen-1-i:].T).sum(axis=0)
        ...
        for i in range(blen):
            for j in range(a.shape[0]):
                for k in range(d.shape[0]):
                    res_prob[i, j, k] += binom.pmf(d[k] - c_val, c_val, params['p3']).dot(poisson.pmf(c_val, a[j] * params['p1'] + b[i] * params['p2']))
    else:
        for i in range(blen):
            for j in range(a.shape[0]):
                for k in range(d.shape[0]):
...
def Expected(prob_f, params, model):
    prob, val = prob_f(params, model)
    return (val * prob).sum()


def Variance(prob_f, params, model):
    prob, val = prob_f(params, model)
    E = Expected(prob_f, params, model)
    return ((val ** 2) * prob).sum() - E ** 2
